...
const {WELCOME_SET,EXIT_MSG,FOOTER,BOT_INFO,INSTAGRAM} = await getVar();
	...
	try {
      ...
      let Wlcm = await getBuffer(ppuser);
                    	if (m.action == 'add') {
                        let mension = {key: {fromMe: false,"participant":"0@s.whatsapp.net", "remoteJid": "94715168503@g.us"}, "message": {orderMessage: {itemCount: 9999999,status: 200, thumbnail: Wlcm, surface: 200, message: `${subject}`, orderTitle: 'Inrl', sellerJid: '0@s.whatsapp.net'}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true}
